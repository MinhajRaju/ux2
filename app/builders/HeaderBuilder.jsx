...
// Default data for a brand-new header template
...
export default function HeaderBuilder({ templateName, initialData, onSave, onBack }) {
  const [isMobile, setIsMobile] = useState(false);
  const [preview,  setPreview]  = useState(false);
  const [saving,   setSaving]   = useState(false);

  const sections      = useHeaderStore(s => s.sections);
  const selectedId    = useHeaderStore(s => s.selectedId);
  const selectionMeta = useHeaderStore(s => s.selectionMeta);
  const setSelected   = useHeaderStore(s => s.setSelected);
  const clearSelected = useHeaderStore(s => s.clearSelected);
  const loadData      = useHeaderStore(s => s.loadData);
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f4f6fb' }}>
        <div style={{ background: '#0f172a', padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ color: '#94a3b8', fontSize: 12 }}>Header Preview — {templateName || 'Header'}</span>
        ...
        <StructureRenderer data={{ sections }} mode="view" elementMap={headerCanvasConfig.elementMap} />
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      <div style={{ height: 52, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', background: '#fff', borderBottom: `1px solid ${T.border}`, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 5, background: '#eef2ff', color: '#4338ca', fontWeight: 800 }}>⊤ HEADER</span>
          <span style={{ fontSize: 13, fontWeight: 700, color: T.text }}>{templateName || 'Header Builder'}</span>
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        <LeftPanel elementMap={headerCanvasConfig.elementMap} sections={sections} selectedId={selectedId} onSelectSection={(id) => setSelected(id, { mode: 'section' })} onSelectElem={(id, meta) => setSelected(id, { ...meta, mode: 'element' })} onSelectCol={(id, meta) => setSelected(id, { ...meta, mode: 'col' })} singleSection={headerCanvasConfig.singleSection} />
        <Canvas store={useHeaderStore} config={headerCanvasConfig} isMobile={isMobile} />
        {selectedId && selectionMeta && <RightPanel store={useHeaderStore} elementMap={headerCanvasConfig.elementMap} onClose={clearSelected} />}
      </div>
    </div>
  );
}